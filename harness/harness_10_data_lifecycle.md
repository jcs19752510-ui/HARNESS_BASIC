...
> "저장 시 암호화 여부" 컬럼은 백업본이 아니라 **운영 중인 원본 저장소**
> 기준입니다 — 백업본 암호화는 `harness_20_backup_dr.md §1`에서 별도로
> 다룹니다. 이 컬럼이 비어 있으면 "암호화 안 함"이 아니라 "검토 안 함"으로
> 간주되므로, 암호화하지 않기로 결정했다면 그 사실과 근거를 명시적으로
> 적으세요(빈칸 금지 — `harness_00 §4-1` "되돌리기 어려운 결정" 원칙 적용).

| 데이터 항목 | 민감도 | 보존기간 | 저장 시 암호화 여부 | 파기 트리거 | 근거 |
|---|---|---|---|---|---|
