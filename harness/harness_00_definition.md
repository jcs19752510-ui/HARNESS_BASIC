...
> harness v1.18 · 문서 22종 · 특정 프로젝트/도메인에 종속되지 않는 범용 프레임워크
...
| 🏗️ 설계 | 어떻게 만들 것인가 | `08_adr` · `13_tech_conventions` · `10_data_lifecycle` · `21_ux_design_process` |
...
| `21_ux_design_process` | UX/디자인 프로세스 | 설계 | 🔴 Critical | 화면설계 확정·프론트컨벤션·접근성·Design QA |
...
**Phase A · 1회만 — 프로젝트 착수**
1. 요구사항 + 비기능요구사항(동시성·권한·감사·개인정보·성능·보안) 정의
2. 화면설계(정보구조/와이어프레임) 확정 → 사람 승인
3. 데이터 모델(ERD) 확정 → ADR 기록
4. 전역 기술 컨벤션 확정 → `13_tech_conventions`
5. 마스터 TRD 작성
6. 릴리스 우선순위 확정 → `07_release_planning` (위험도 채점표 §3-1 사용)
7. 위험도 기준 실행 순서 확정
...
```mermaid
flowchart TD
    A1[요구사항 정의] --> A1b[화면설계 확정 - IA/와이어프레임, 사람 승인]
    A1b --> A2[ADR - 데이터모델 확정]
    A2 --> A3[전역 기술컨벤션 확정]
    A3 --> A4[마스터 TRD 작성]
    A4 --> A5[릴리스 우선순위 확정]
    A5 --> A6[위험도 기준 실행순서 확정]
    A6 --> B0{단위별 실행 루프}
    B0 --> B1[단위 TRD + AC 확정]
    B1 --> B2[작업지시서 작성]
    B2 --> B3[프롬프트 생성 - AI 실행]
...
5개 레이어 + 거버넌스 백본을 문서 22종이 커버합니다. 하지만 v1.2~v1.18에서
...
v1.5(2026-09-07)에서 Phase A 화면설계 단계 신설·위험도 채점기준표·프론트/
접근성 컨벤션 등 15건을, v1.6(2026-09-08)에서 그 프론트/접근성 컨벤션을
`harness_21_ux_design_process.md`로 분리하고 신설 필드들에 기본값을 채워
넣었습니다 — 이번엔 20년차 기획/설계/개발/디자이너 관점 갭 분석이었습니다.
v1.7(2026-09-08)에서 Claude Code 전용 가속 도구 4종(`.claude/skills`·
`.claude/agents`)과 "실제로 실행해서 확인했는가" 품질게이트를, v1.8(같은 날)
에서 플러그인 배포 사본·도메인중립 감지 훅·주간 감사 클라우드 routine을
추가했습니다. v1.9(2026-09-08)는 보안 중심 2차 갭 분석으로 간접 프롬프트
인젝션 방어 원칙·인증보안 체크리스트·PII 암호화 컬럼·Security RACI 역할·
CODEOWNERS/dependabot/SECURITY.md를 신설했고, v1.10(2026-09-09)은 로거
미설정으로 로그가 침묵하는 실패를 실전에서 발견해 로깅 신뢰성 게이트를,
v1.11(같은 날)은 git 접속정보 관리 원칙을 보강했습니다. v1.12(2026-09-09)는
2026년 업계 표준(OWASP Top 10 for Agentic Applications 등) 대조 갭분석으로
OWASP 대조표를 신설했고, v1.13~v1.15(같은 날)에서 그 표의 미대응 4건
(ASI03/06/07/10)을 전부 개발/실행 인프라 레벨 원칙 신설로 🟡 부분 대응으로
전환한 뒤, 남은 구체값 공백을 `harness_16_tech_debt_log.md`에 TD-001~003으로
정식 등록했습니다. v1.16(같은 날)은 그 작업과 무관하게 발견된 문서 드리프트
3건(이 문서 자체의 버전 헤더가 v1.6에서 9버전 방치된 것, `harness_11 §3`
소유권 다이어그램에 `harness_05`/`15`/`16` 오너 누락, `harness_19 §4`에
`harness_16` 행 누락)을 해소했습니다 — 버전 헤더 건은 v1.6에서 한 번 고쳤다가
재발한 것이라 `harness_06 §3` 실패 패턴으로 신규 등록했습니다. v1.17(같은 날)은
`harness_19 §4` 표에 남아있던 나머지 8개 문서(03/04/06/09/12/14/17/19)를
전부 "1인/팀 무관 필수"로 채워 22종 전체를 이 표가 커버하게 만들었고,
동시에 이 문서(00_definition)도 v1.17로 함께 동기화해 v1.16에서 막 등록한
"버전 헤더 동기화 누락" 재발 방지책을 그 자리에서 바로 실천했습니다.
v1.18(같은 날)은 그때 "나중에 자동검증 대상으로 승격 검토"라고만 적어두고
실행하지 않았던 것을 발견해, `harness_19 §3`에 실제 grep 3종(배지·푸터·
changelog 최신 버전 비교)으로 편입했습니다 — 이 문서도 다시 v1.18로
동기화했습니다.
...
---
`HARNESS-GUIDE · v1.18 · 2026-09-09`
