...
### §3-2. 위험도 점수 → 리뷰 강도 연동 [사람 확정]

> §3-1 점수를 "8점 이상이면 ADR 작성"에만 쓰면, 4점짜리 단위와 7점짜리
> 단위의 리뷰 강도가 똑같아진다. EU AI Act·NIST AI RMF 등 2026년 기준
> 규제/표준이 요구하는 "위험도에 비례한 사람 감독(tiered oversight)"에
> 맞춰, 점수를 실제 리뷰 강도에 연동한다 — 이렇게 하면 나중에 "위험도에
> 따라 감독 강도를 다르게 했다"는 걸 문서로 증명할 수 있다.

| 점수 구간 | 등급 | 리뷰 강도 | 비고 |
|---|---|---|---|
| 4~6점 | 낮은 위험 | AI 자가검증(AC pass/fail 자체 보고) + 사람 스팟체크 | `harness_00_overview §6` 품질 게이트로 충분 |
| 7~9점 | 중간 위험 | 사람 diff 리뷰 1인 필수 (기존 기본값) | `harness_05 §1` 머지 조건과 동일 |
| 10~12점 | 높은 위험 | 사람 diff 리뷰 2인 이상 + `harness_08` ADR 필수 | 8점 이상 ADR 규칙을 포함하며, 10점 이상은 리뷰 인원도 늘림 |

- 이 표는 `harness_05 §1` "머지 조건"의 "사람의 diff 리뷰 완료"를 구체화한
  것입니다 — 팀(2인 이상) 프로젝트에 그대로 적용합니다.
- **1인 프로젝트**: 2인 이상 검토가 물리적으로 불가능하므로, 10~12점
  항목은 대신 최소 하루 이상 시간 간격을 두고 재검토하세요(`harness_11 §4`
  모자 전환 태그로 "[리뷰]"를 다시 찍음) — 같은 사람이 같은 순간의 판단을
  두 번 확인하는 착시를 방지하기 위함입니다.
