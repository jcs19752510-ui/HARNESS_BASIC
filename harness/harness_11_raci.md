...
## §6. 외부 이해관계자 보고 주기 [사람 작성]
> §1~§5는 프로젝트 내부 역할(PO/TL/Reviewer 등) 간 관계만 다룹니다. 발주처나
> 실사용자 대표처럼 **프로젝트 밖의 이해관계자**에게 언제·어떤 형식으로
> 진행상황을 보여줄지가 없으면, "다 만들고 나서야 방향이 달랐다는 걸 안다"는
> 전형적인 실패로 이어집니다. 1인 프로젝트에서도 발주처가 있다면 이 절은
> 채우세요 — 없으면 "해당 없음"이라도 명시하세요.

- 외부 이해관계자 목록과 관심사: 프로젝트별 실제 대상자로 채우되, 최소
  "발주처(또는 의사결정권자) — 일정/예산/범위"와 "실사용자 대표 — 사용성/
  실무 적합성" 두 유형은 기본으로 포함
- 보고 주기: 마일스톤 완료마다(필수) + 진행 중인 마일스톤이 2주 이상 걸리면
  중간에 최소 1회 추가 보고 (기본값 — 프로젝트 특성에 따라 조정)
- 보고 형식/채널: 데모(가능하면 실제 동작 화면) + 1페이지 요약 문서
- 보고 항목: MVP 범위(`harness_07 §2`) 대비 진행률(`harness_07 §4`), 주요 의사결정 사항, 다음 마일스톤까지 필요한 것
- 보고 내용에 대한 외부 이해관계자의 피드백을 반영해야 한다면 `harness_12_change_management.md` 변경관리 절차로 처리
