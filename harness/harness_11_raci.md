...
## §4. 1인 프로젝트 — "모자 전환" 실전 습관 (2026-09-05 추가)

> §1의 경고("역할 전환 시점을 스스로 명시하세요")가 지금까지 실전에서 안
> 지켜졌음이 감사에서 확인됨 — 말로만 있었지 실제로 기록한 적이 없었다.
> 문서를 늘리는 대신, 커밋 메시지/승인 메시지에 붙이는 **한 단어 태그**로
> 대체한다 (별도 문서 작성 부담 없이 습관화 가능).

| 태그 | 의미 | 예시 |
|---|---|---|
| `[기획]` | Product Owner 모자 — 범위/우선순위 결정 | "[기획] A7을 MVP에 포함하기로 함" |
| `[설계]` | Tech Lead 모자 — ADR/데이터모델/기술컨벤션 승인 | "[설계] ADR-008 후보 B 채택" |
| `[리뷰]` | Reviewer 모자 — diff/AC pass-fail 판정 | "[리뷰] AC-1~11 전부 pass 확인, merge 승인" |
| `[배포]` | Deploy Approver 모자 — 배포/롤백 결정 | "[배포] PROD_SCH push 승인" |
