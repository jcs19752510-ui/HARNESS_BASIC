...
- [ ] 실제로 staging에서 구동해 핵심 흐름을 손으로 확인했는가 (테스트 통과와
      실제 동작 확인은 별개 — `harness_00_overview §6`과 동일 기준)
- [ ] staging/실제 배포 환경(예: 컨테이너)에서 로그가 의도한 레벨까지 실제로
      출력되는지 확인했는가 — 로컬 개발환경에서만 확인하고 넘어가면, 배포
      환경의 로깅 설정이 로컬과 달라서 재발할 수 있다 (`harness_06 §3` "로거
      미설정으로 인한 침묵 로그" 참조)
- [ ] (해당 시) 성능 회귀 확인 — TRD §0-1에 성능 기준이 명시된 단위라면 그
      기준 대비 응답시간/처리량이 악화되지 않았는가
- [ ] (화면이 있는 단위라면) 접근성 체크리스트 통과 (`harness_21 §5`)
- [ ] (화면이 있는 단위라면) Design QA 통과 — 구현 결과가 Phase A-2 화면설계
      산출물과 일치하는가 (`harness_00_overview §6`과 동일 항목, 배포
      직전 재확인)
