...
- MCP 서버 연동: (사용 중인 MCP 서버가 있다면 이름/용도/설정 위치를 기록 —
  ...
  - 도입 전 출처/권한 범위 검토자: MCP 서버는 임의 코드 실행·외부 네트워크
    ...
    또는 Tech Lead
  - 도입 후 지속 점검 [빈칸 금지 — 도입 시 1회 검토로 끝내지 않음]: MCP
    서버가 제공하는 도구 설명(tool description)이 이전 세션과 달라졌는지
    주기적으로(예: 새 세션 시작마다 또는 최소 주 1회) 대조한다. 도구
    설명이 조용히 바뀌어 숨은 지시가 주입되는 공격(tool poisoning)은
    최초 도입 심사만으로는 못 잡는다 — 대조 방법과 주기를 명시할 것
  - 파괴적/비가역적 동작 승인: 파일 삭제·메일 발송·결제 처리처럼 되돌리기
    어려운 동작을 하는 MCP 도구는 일반 도구와 동일하게 취급하지 않는다 —
    `harness_05 §3` "위험 명령 차단"과 동일한 원칙으로, 실행 전 매번 사람
    승인을 거치도록 등록한다(Claude Code 사용자는 `.claude/settings.json`의
    `permissions.ask`/`deny`에 해당 MCP 도구 패턴 추가)
  - 암호화 통신 필수: MCP 서버와 그 뒤의 실제 시스템(DB 등) 간 통신이
    암호화(TLS 등)되지 않으면 도입 대상에서 제외한다
